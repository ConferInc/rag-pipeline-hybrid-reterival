...
| AN-9 | As a user, at breakfast time (7-9 AM local), I receive a proactive suggestion: "Good morning! How about **Spinach & Egg Scramble** for breakfast? It's high in protein and matches your goals" — where the recipe is pulled from the RAG recommendation system based on my profile, preferences, and allergens | P1 | Proactive Meal Suggestion |
| AN-10 | As a user, around lunch time (11:30 AM-12:30 PM local), if I haven't logged lunch yet, I receive: "Lunchtime! Based on your goals, try **Grilled Chicken Salad** — it fits within your calorie budget" — with a direct link to the recipe and a "Log This Meal" action button | P1 | Proactive Meal Suggestion |
...
| `suggest_breakfast` | No breakfast logged yet + time is 7-9 AM local → call RAG `/meal-candidates` with `meal_type=breakfast`, user profile, allergens → pick top recipe | 7-9 AM local |
| `suggest_lunch` | No lunch logged yet + time is 11:30 AM-12:30 PM local → call RAG `/meal-candidates` with `meal_type=lunch`, user profile, nutrition gaps → pick top recipe | 11:30 AM-12:30 PM local |
...
- [ ] Proactive breakfast suggestion fires between 7-9 AM local with a recipe from RAG `/meal-candidates`
- [ ] Proactive lunch suggestion fires between 11:30 AM-12:30 PM local if no lunch logged, with "Log This Meal" action
- [ ] Suggested recipes respect user allergens, dietary preferences, and nutrition goals
